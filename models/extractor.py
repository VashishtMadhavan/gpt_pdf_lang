...
class ExtractionModel(BaseDocQAModel):
    ...
    def __init__(self, format_model: Type[BaseModel], 
                 find_matches: bool = True, **kwargs) -> None:
    ...
    def _post_process_answers(self, page_content: str, result: Any) -> Tuple[List[Tuple[int, int]], List[str]]:
        offsets = []
        entities = []
    ...
    def run(self, docs: List[Document]) -> List[ExtractionResult]:
        ...
        for doc in docs:
            ...
            offsets, entities = self._post_process_answers(doc.page_content, parsed_result)
            if len(entities) != 0:
                output.append(ExtractionResult(page_id=doc.metadata["page"], 
                                               source=doc.metadata["source"],
                                               offsets=offsets, entities=entities))
        return output